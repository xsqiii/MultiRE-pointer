...
class Vocab(object):
  ...
  def __init__(self, vocab_file, max_size):
    ...
    # todo read word from vocab_file
    with open(vocab_file, 'r') as vocab_f:
      lines = vocab_f.readlines()
      for line in lines:
        line = line.strip()
        # pieces = line.split()     # todo modify Separator
        pieces = line.split('\t')
        if len(pieces) != 2:
...
def example_generator(data_path, single_pass):
  ...
  Args:
    data_path:
      Path to tf.Example data files. Can include wildcards, e.g. if you have several training data chunk files train_001.bin,
      train_002.bin, etc, then pass data_path=train_* to access them all.
    single_pass:
      Boolean. If True, go through the dataset exactly once, generating examples in the order they appear, then return.
...
# todo for webnlg dataset
def article2ids_webnlg(article_words, vocab):
  """Map the article words to their ids. Also return a list of OOVs in the article.
...
# For webnlg dataset
def abstract2ids_webnlg(abstract_words, vocab, article_oovs):
  """Map the abstract words to their ids. In-article OOVs are mapped to their temporary OOV numbers.
...
# For webnlg dataset
def outputids2words_webnlg(id_list, vocab, article_oovs):
  """Maps output ids to words, including mapping in-article OOVs from their temporary ids to the original OOV string
  (applicable in pointer-generator mode).
