...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, vocab, hps):
    ...
    # tf.logging.info(article_words)

    # Process the abstract
    abstract = abstract_sentences.strip() # string
    abstract_words = abstract.split() # list of strings

    abs_ids = [vocab.word2id(w) for w in abstract_words] # list of word ids; OOVs are represented by the id for UNK token
    # abs_ids = [int(w) for w in abstract_words]
    ...
    if hps.pointer_gen:
      ...
      self.enc_input_extend_vocab, self.article_oovs = data.article2ids_webnlg(article_words, vocab)
      # Get a verison of the reference summary where in-article OOVs are represented by their temporary article OOV id
      abs_ids_extend_vocab = data.abstract2ids_webnlg(abstract_words, vocab, self.article_oovs)
...
class Batch(object):
  ...
  def store_orig_strings(self, example_list):
    ...
    # tf.logging.info('original_abstracts:%s', self.original_abstracts)
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    with open(self._data_path, 'r') as datas:
      lines = datas.readlines()
      for line in lines:
        line = line.split('\t')
        if len(line) != 2:
          print(line)
          raise Exception('The format of this line is Wrong.')
        else:
          sent = line[0].strip()
          tag = line[1].strip()
          example = Example(sent, tag, self._vocab, self._hps)
